...
"""
主要修改原始Unet编码和解码部分，原始Unet使用卷积+relu,DoubleConv1，DoubleConv放在上采样层(原来做了两次卷积,一次反卷积,一次上采样)
选择反卷积用DoubleConv,选择上采样用DoubleConv1,类似残差的轻量化改进
"""
...
class DoubleConv(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            ...
            # 这里跟一个新的激活函数gelu(2022年提出平缓度比relu好在零点处可导)
            nn.GELU(),
            # 优化信息流加边归一化
    ...
    # x为输入图,将x送入conv(主干),再将x送入C3(侧分支)，最后将得到的out和out1一起送进上面的self.GELU = nn.GELU()进行信息流优化
    def forward(self, x):
        out = self.conv(x)
        out1 = self.C3(x)
        _x = self.GELU(out1+out)
        return _x
